...
def _process_data(df, sort_by, sort_categories_by, subset_size, sum_over, intersection_limit=0):
    df, agg = _aggregate_data(df, subset_size, sum_over)
    total = df.sum()
    df = _check_index(df)

    totals = [np.sum(df[df.index.get_level_values(name).values.astype(bool)].sum())
              for name in df.index.names] #yollct
    ...
    df = df.reindex(agg.index.values)
    ...
    df['_bin'] = pd.Series(df_packed).map(
        pd.Series(np.arange(1, len(data_packed)+1),
                  index=data_packed))
...
class UpSet:
    ...
    def __init__(self, data, orientation='horizontal', sort_by='degree',
                 ...
                 show_counts='', show_percentages=False, intersection_limit=0):
