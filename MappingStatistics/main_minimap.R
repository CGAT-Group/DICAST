...
bam <- readBamMinimap(bam_path,"+")
genomic_reads <- loadReferenceReads(genomic_reads_data,fastq_path,gtf_path)

print('changing mate number of reads on "-" stranded transcripts')
old <- Sys.time()
transcriptStrands <- as.character(strand(genomic_reads[names(bam)]))
change_mate_number <- transcriptStrands == "-"
bam[change_mate_number]$mate <- bam[change_mate_number]$mate - 2
bam[bam$mate==0]$mate <- 1
bam[bam$mate==-1]$mate <- 2
names(bam) <- paste(bam$id,bam$mate,sep="_")
print(Sys.time()-old)
