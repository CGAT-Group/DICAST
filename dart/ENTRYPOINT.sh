...
dart -i /myvol1/dartindex/ -f ${line}?.fastq  -o ${line}dart.sam
...
if ! test -d "myvol1/dartindex"; then build_index; fi

build_index() {
#Build Genome index
# find gtf and da files with these  $(find /myvol1/ -name "*.fa")  $(find /myvol1/ -name "*.gtf")
bwt_index /myvol1/Homo_sapiens.GRCh38.dna.primary_assembly.fa /myvol1/dartindex/
}
...
#...tag outputs with this flag to name it per fastqfile         "${line##*/}"
#...address for all gtf files are                               $(find /myvol1/ -name "*.gtf")
dart -i /myvol1/dartindex/ -f ${line}1.fastq -f2 ${line}2.fastq -o ${line}dart.sam
